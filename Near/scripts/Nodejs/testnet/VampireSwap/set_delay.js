const HELP = `Please run this script in the following format:
    node extend_operators.js owner lock_delay
`;
...
async function main(deployerAccount) {
    ...
    await FARMING.modify_lock_delay({
        args: {
            lock_delay: parseInt(process.argv[3]),
        },
        amount: "1",
    });
    console.log("New lock delay:", await FARMING.get_lock_delay());
}
...
async function initializeFarming(deployer, farming) {
  const FARMING = new Contract(
      ...
      farming,
      {
        ...
        changeMethods: [ "modify_lock_delay" ],
        sender: deployer
      }
  );
  return FARMING;
